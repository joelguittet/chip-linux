		      ...
		      const void *buf, int offset, int len)
{
	...
	if (pnum >= 0) {
		...
		if (full) {
			...
			ret = ubi_coso_add_full_leb(ubi, vol_id, lnum, 0, vol->data_pad);
			if (ret)
				ubi_warn(ubi,
...
retry:
	...
	if (full) {
		err = ubi_coso_add_full_leb(ubi, vol_id, lnum, 0, vol->data_pad);
		if (err)
			ubi_warn(ubi,
				 "failed to add LEB %d:%d to the full LEB list",
				 vol_id, lnum);
...
retry:
	...
	err = ubi_coso_add_full_leb(ubi, vol_id, lnum, data_size, vol->data_pad);
	if (err)
...
retry:
	...
	if (full) {
		...
		ret = ubi_coso_add_full_leb(ubi, vol_id, lnum, len, vol->data_pad);
		if (ret)
			ubi_warn(ubi,
				"failed to add LEB %d:%d to the full LEB list",
				vol_id, lnum);
	...
	for (i = 0; i < num_volumes; i++) {
		...
		ubi_rb_for_each_entry(rb, aeb, &av->root, rb) {
			if (aeb->desc.lnum >= vol->reserved_lebs) {
				...
				if (aeb->full) {
					//XXX data_size, data_pad!
					ubi_coso_add_full_leb(ubi, vol->vol_id,
							      aeb->desc.lnum, 0, av->data_pad);
				}
			}
		}
	}
